...
$(function	()	{
    tableObj = $table.DataTable({
		...
		createdRow: function ( row, data, index ) {
            var operate = '<a class="btn btn-info btn-sm" href="adDetail?adID='+data.adID+'&type=view">查看详情</a>';
            $('td',row).eq(0).text(index+1);
            $('td',row).eq(3).text(checkadShape(data.adShape));            
            $('td',row).eq(5).text(checkStatus(data.adStatus));
            $('td',row).eq(7).text(setTimes(data.createTime));
            if(data.adStatus!="1" && data.adStatus!="2" && data.adStatus!="3" && data.adStatus!="4"){
            	operate+='<a class="btn btn-warning btn-sm" href="adDetail?adID='+data.adID+'&type=check">处理申请</a>';
            }
            if(data.adStatus=="3"){
...
//点击开启投放、关闭投放
function change(data,button){
   var  adStatus = data.adStatus=='1'?'3':'1',
        reason = data.adStatus== '1'?'管理员强制暂停':'管理员开启投放';
    $.ajax({
        url:'handleAdApply',
        type:'post',
        dataType:'json',
